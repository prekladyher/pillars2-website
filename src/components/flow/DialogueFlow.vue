...
const { conversation, currentNode } = storeToRefs(conversationStore);

const entry = ref<ConversationNodeType>();
...
watch(highlightPath, path => {
  if (!path) {
    ...
    const entries = [path.origin, ...path.steps.map(step => step.entry)] as ConversationNodeType[];
